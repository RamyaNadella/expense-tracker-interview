...
- Failure artifacts are enabled in config:
...
## Backend Env Note (JWT_SECRET)
- Frontend E2E tests do not use backend Vitest setup files.
- The backend started by Playwright `webServer` reads environment variables from the runtime process.
- `frontend/playwright.config.ts` explicitly passes `JWT_SECRET` to the backend `webServer` process, so E2E runs do not depend on auth fallback behavior.

## Debugging Tips
- Use `--headed --workers=1` when investigating flaky behavior.
- Inspect traces and HTML report for timing/navigation issues.
- Inspect per-test `test-results/*/test-failed-1.png` and `test-results/*/video.webm` for failed tests.
